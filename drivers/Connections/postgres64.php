...
class postgres64 extends ADOConnection{
	...
	*/
	...
	function _connect($str,$user='',$pwd='',$db='',$ctype=0)
	{
		...
		if ($user || $pwd || $db) {
			$user = $this->adodb_addslashes($user);
			$pwd = $this->adodb_addslashes($pwd);
			if (strlen($db) == 0) $db = 'template1';
			$db = $this->adodb_addslashes($db);
			if ($str)  {
				$host = explode(":", $str);
				if ($host[0]) $str = "host=".$this->adodb_addslashes($host[0]);
				else $str = '';
